...
def parse_arguments():
    ...
    parser.add_argument("--dfm_type", type=str, default="campbell", choices=["campbell", "campbell_rate_matrix"],
                        help="DFM type: 'campbell' or 'campbell_rate_matrix'")
    ...
    parser.add_argument("--dataset_name", type=str, default="qme14s",
                        choices=["qm9", "qme14s"],
                        help="Dataset name for conditioning properties")
...
def main():
    ...
    # Use ClassifierFreeGuidance class instead of FlowMol
    model = ClassifierFreeGuidance.load_from_checkpoint(args.model_checkpoint)
    model.to(device)
    model.eval()
