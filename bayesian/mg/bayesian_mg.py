...
search_space = [Real(0.0, 3.0, name='guide_w')] 
...
if dataset_name == 'qm9':
    ...
    qm9_val_path = (
        "../../sampling_input/qm9/in_distribution_sampling"
    )
    ...
    qme14s_val_path = (
        "../../sampling_input/qme14s/in_distribution_sampling"
    )
...
def objective(guide_w):
    foldername = (
        f"{method}_{property_name}/"
        f"bayes_opt_w{guide_w:.2f}"
    )
    sdf_file = (
...
with open(f"{args.out_prefix}/{method}_results_{property_name}.pkl", "wb") as pf:
