...
from molguidance.utils.dataset_stats import compute_p_c_given_a
...
def process_all_molecules(dataset_config, args):
    ...
    sdf_file = raw_dir / 'cleaned_qme14s.sdf'
...
if __name__ == "__main__":
    ...
    if dataset_config['dataset_name'] != 'qme14s':
        raise ValueError('This script only works with the qme14s dataset')

    ##########3
    # this must be changed for the qme14s dataset
    ############3

    # get qme14s csv file as a pandas dataframe
    qme14s_csv_file = Path(dataset_config['raw_data_dir']) / 'dipole_moments_cleaned.csv'
